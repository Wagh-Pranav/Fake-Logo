...
import re
from detection.files import common
import imageio
from pathlib import Path
# Build paths inside the project like this: BASE_DIR / 'subdir'.
BASE_DIR = Path(__file__).resolve().parent.parent

train_dir = str(BASE_DIR) + '\\detection\\files\\flickr_logos_27_dataset'
...
tf.app.flags.DEFINE_integer("patch_size", 5, "A patch size of convolution filter")
    ...
    if(request.session.has_key('account_id')):
        ...
        if request.method == 'POST':
            ...
            with graph.as_default():
                ...
                # restore weights
                f = str(BASE_DIR) + "\\detection\\files\\weights.npz"
                if os.path.exists(f):
            ...
            with tf.Session(graph=graph) as session:
                ...
                content['class_name'] = "Class name:", common.CLASS_NAME[np.argmax(pred)]
                content['probability'] = "Probability:", np.max(pred)
                print("Class name:", common.CLASS_NAME[np.argmax(pred)])
                print("Probability:", np.max(pred))
        return render(request, 'admin/detect.html', content)
    else:
        return HttpResponseRedirect(reverse('account-login'))
