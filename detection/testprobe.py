...
for u in uarray:
    for th in tharray:
        ...
        for line in lines:
            ...
            if probeI.shape[0] == 0 or probeI.shape[1] == 0:
                print("Error: the input image is empty")
        ...
        for brands in logoMatchList:
            if (brands==[]):
                if (actualBrand[i]=='none'):
                    ...
                    fn=fn+1
            i=i+1
        print ("accuracy =" + str(       pt*   (         (float)(tp+tn)    /    (float)(tp+tn+fp+fn)                ))             )